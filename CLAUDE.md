...
out of `tmp/` on 2026-07-21, so this experiment's code and its own outputs live together,
...
- **`dnm_training_size/run_dnm_training_experiment.py`** — `-mode validate` runs
  ...
  cached in the repo-root `tmp/` (`-cache_dir`, shared with
...
files or the duckdb join — only the training-set tables already cached in `tmp/` — so it
