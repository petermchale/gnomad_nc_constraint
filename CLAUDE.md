...
| `misc/genomic_features13_sel.txt` | 19 KB | Per-context selected features + fitted coefficients: `context, feature, window, coef, se, pval`. |
| `fig_tables/genomic_features13_sel.annot.txt` | small | Same as above plus `ft_sel` (bool, Bonferroni-selected) and `label`. |
| `fig_tables/mutation_rate_by_context_methyl.txt` | 12.5 KB | Per-trinucleotide-context (× CpG methylation level) fitted mutation rate: `context, ref, alt, methylation_level, possible, observed, proportion_observed, mu, fitted_po`. **This is the step-1 (context-only) mutation-rate table.** |
