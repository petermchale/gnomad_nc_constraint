...
def check_dnm1(con, sites: str, features: str) -> bool:
...
def check_dnm0(con, sites: str) -> bool:
...
def check_join_is_lossless(con, sites: str, features: str) -> bool:
    """
...
def check_3mer_is_step1_rate(con, sites: str, rate_path: str) -> bool:
    """
...
def check_feature_shape(con, features: str) -> bool:
...
def main() -> None:
    ...
    results = {
        "dnm1 count reproduces the paper": check_dnm1(
            con, paths["dnm1_sites"], paths["dnm1_features"]),
        "dnm0 count reproduces the paper": check_dnm0(con, paths["dnm0_sites"]),
        "the training join is lossless": check_join_is_lossless(
            con, paths["dnm1_sites"], paths["dnm1_features"]),
        "`3mer` is the step-1 rate": check_3mer_is_step1_rate(
            con, paths["dnm1_sites"], rate_path),
        "feature table has the expected shape": check_feature_shape(
            con, paths["dnm1_features"]),
    }
    ...
    if all(results.values()):
        print("\nThe four shipped tables ARE the training set the paper describes. Both "
              "published counts are reproduced -- by the dnm1 FEATURE table and the dnm0 "
              "SITE table respectively, so check any count against the right one of the "
              "four before concluding a table is short.")
    else:
        raise SystemExit("\nAt least one claim about the training set does not hold; "
                         "the identity of the training set is a precondition for every "
                         "fig5 claim about what step 2 was fit on.")
