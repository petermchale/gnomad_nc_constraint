...
md(r"""
## Panel F — the same fix, in the units the score is used in
...
here with the other label-free panels rather than with the discovery analyses that follow.
And it is computed from **panel E's own table and bins**, so E and F are two views of one fix
on one population — E's rank returning to 0.5, F's percentile flattening — rather than two
...
where the genome actually is, and the same mark panel E carries. The horizontal one is the
matched genome-wide calling rate, and it is **this panel's null**: a score meeting its own
promise would put a fixed $z$ at a fixed percentile of every bin, so it would *be* that line.
It belongs to both curves, the matching pinning both to it, and to neither in particular.

Where each curve meets it is the content. The retrained curve hugs it through the GC bulk and
falls away in the tails, since what is pinned is the window-weighted *mean* rather than any
...
fig, ax = plt.subplots(figsize=FIGSIZE)
# PANEL E'S OWN MEAN, not a freshly computed one: F is drawn on df_e, so the two panels must
# mark the same place on the GC axis or the "one measurement seen twice" pairing is only
# approximately true.
#
# AND THE MATCHED RATE AS A HORIZONTAL LINE, which is the panel's null rather than a summary
# of it: both thresholds were chosen to call that fraction of the whole population, so a score
# whose z meant the same thing everywhere would lie along it. It is the genome-wide value,
# over every window including the bins min_n drops, and NOT a mean over the plotted points.
panels.panel_calling_rate(ax, rate_f, thr_f, xrange=XRANGE,
                          matched_rate=matched_f,
                          gc_mean=float(df_e["GC_content"].mean()))
save(fig, "F")

# THE PER-BIN VALUES THE CAPTION QUOTES. calling_rate_by_gc prints only the min and max
# across bins, and its minimum is 0.000% from bins that call nothing -- true, and not what a
# caption wants for "the most AT-rich bin drawn". Percentile = 100 x (1 - rate) is exactly
# panel F's y axis, so these come off the arithmetic the panel draws, not off the PDF.
...
md(r"""
## Supporting Figure 8 — what the Fig. 5 fix does to *discovery*

Fig. 5E and 5F say the decontaminated score is no longer GC-biased. Neither says whether the
biased score was nevertheless the better **detector**: bias and signal-to-noise act on discovery
jointly (McHale et al.'s Fig. 3), and only one of them changed. This figure is that test, built
as their **Fig. 4A/B**. Call a window constrained when Gnocchi clears a threshold, label it
constrained if it overlaps a GeneHancer enhancer, and read performance *within each GC bin*,
with two Gnocchi variants in place of their four metrics.

*Bare letters mean this figure's panels; Fig. 5's are written 5A–5F. Every number below is from
the committed run.*

| | Asks | Quantity | Operating point |
|---|---|---|---|
| **A**, **B** | Where does each score send its calls, and is a call worth more there? | recall and $\mathrm{LR}^{+}$ per bin, one panel per score | one fixed global threshold per score |
| **C** | Does decontaminating change the ranking at all? | auPRC / positive-class fraction | none |
| **D**, **E**, **F** | What threshold would each score need to call the top **99%**, **50%**, **1%** of every bin? | per-bin threshold, no labels used | — |
| **G**, **H**, **I** | At each of those depths, does decontaminating raise what a call is worth? | $\mathrm{LR}^{+}$ ratio per bin | calling rate matched **within** each bin |

### The measure, and the two operating points

A call's **evidential weight** is the positive likelihood ratio
...
population, which is the genome-wide matching Fig. 5F uses. Each *bin*'s share is then free,
and that freedom **is** the bias, so imposing a per-bin rate here would delete what A and B
measure. **D–I do the opposite**, matching the calling rate *within* each bin at 99%, 50% and
1% (`data.LAX_CALL_RATES`) — the left tail, the median and the right tail of each bin's own
score distribution. Matching inside the bin isolates ranking from threshold placement, so
$z = 4$ enters this figure only as A's actual threshold.

**Error bars follow from that.** C and G–I carry a 95% paired-bootstrap interval on the
decontaminated curve relative to published: one index vector per replicate, *both* models
scored on it, so the variability in which windows a bin happens to contain cancels in the
difference. A and B carry Wilson intervals on each level instead, sitting as they do at two
different operating points. D–F carry none, a quantile of a million windows having none worth
drawing.

### The truth set, and which way it leans

*Lax* is McHale et al.'s own word: GeneHancer covers 18.4% of the noncoding genome against
perhaps 4.51% under human-specific selection, so it buys size at the cost of label confidence,
and 30.9% of windows here are positive. (Their stringent set is deliberately not built;
CLAUDE.md carries the decision.) Enhancer-overlapping windows are kept here as the positive
class, so the `scored` refit is *fit* on the neutral windows alone and *evaluated* on both
halves. **Panel C alone downsamples the positive class**, pegging every bin to the same
prevalence (0.0833), which is what makes one dashed baseline valid across its bins, as in
McHale et al.'s Fig. 4A.

Those positives are GC-rich and published is the GC-biased score, so the truth set hands
**published** a tailwind, between bins and still within them. The decontaminated score wins all
five bins at the top 1% anyway, so **panel I is conservative, not flattered**.

### A and B: discovery is redistributed, not lost

Published's two curves run in **opposite directions**: recall climbs $45.7\times$ with GC while
$\mathrm{LR}^{+}$ falls, so the score calls most where a call is worth least. The decontaminated
score's recall is flat, on the same budget of 10,051 calls (1.00% of the 1,003,036 drawn).
...
Published puts **2,618** calls where $\mathrm{LR}^{+}$ is **1.25** and **38** where it is
**2.06**; the decontaminated score cuts the first to **154** and raises the second to **168**.
The budget being fixed, it spends elsewhere rather than spending less, so "it costs recall" is
a high-GC half-truth.

Two readings to avoid. B's $\mathrm{LR}^{+}$ still declines, $3.11 \to 1.50$, and **that
residual is C's signal-to-noise, not a remnant of the bias**. And comparing the two panels'
$\mathrm{LR}^{+}$ *levels* is a different matter from the redistribution above: the scores sit
at different operating points in every bin, which is the confound G–I remove. The
redistribution claim reads each score's calls against its own recall curve and is untouched by
that.

### C: averaged over all thresholds, decontaminating changes nothing — and that is the finding

Pooled auPRC$/r$ is **1.321** against **1.341**, with the one significant bin *negative*. auPRC
*within* a GC bin cannot see the bias: a GC-dependent bias is nearly a common shift on every
window of a narrow bin, and a common shift cannot reorder a ranking. So the steep decline with
GC — published $1.518 \to 1.199$, decontaminated $1.554 \to 1.298$ — **survives decontamination
intact**, which names it **signal-to-noise** rather than bias. The
observed-against-expected comparison simply carries less constraint signal in GC-rich sequence
relative to its own noise. C is the only panel with **no operating point at all**, so no
threshold placement can be generating that decline, which is why the claim is cited to it.

Two guards. The claim is about the GC **slope** of discriminative power, not its level, which
...
answers with a **curve** at all three depths, the decontaminated score with something close to
a **constant** — so the bias is a shift of the **whole** score within a bin, not a stretched
tail. None of the three uses labels.

| depth | published threshold | decontaminated threshold |
|---|---|---|
| **D**, top 99% | $-5.67 \to -2.98$ | $-4.85 \to -5.20$, spanning 0.79 |
| **E**, top 50% (each bin's median) | $-0.96 \to +2.32$, a swing of **3.28** | $-0.22 \to +0.32$, within **0.54** |
| **F**, top 1% | $2.83 \to 6.38$ (**2.26**-fold) | $3.01 \to 3.04$, spanning 3.01–3.16 |

**E is the cheapest result in the figure and uses no truth set.** The threshold calling the top
half of a bin *is* that bin's median $z$, so in the most GC-rich sequence **half of published
Gnocchi's windows already sit above $z = 2.3$**, Chen et al.'s threshold being only 1.7 further
on, while in the most AT-rich its median sits *below* zero. Read the **swing**, not the offset
from zero: this population retains the enhancer-overlapping windows it classifies, so genuine
constraint displaces a median too. What that cannot produce is the difference between **two
curves scored on identical windows**.

G, H and I measure what those thresholds buy. With the rate matched inside a bin the
$\mathrm{LR}^{+}$ ratio **is** the odds ratio, since
$\text{odds}(\text{precision}) = \mathrm{LR}^{+}\times\text{odds}(r)$ gives

$$\frac{\text{odds}(p_{\text{decontaminated}})}{\text{odds}(p_{\text{published}})}
= \frac{\mathrm{LR}^{+}_{\text{decontaminated}}}{\mathrm{LR}^{+}_{\text{published}}},$$

the prevalence cancelling exactly.

| depth | $\mathrm{LR}^{+}$ ratio, decontaminated / published, per GC bin | favours decontaminated |
|---|---|---|
| **G**, top 99% | $-0.2$, $-0.1$, $-0.2$, $-0.4$, $-0.5$ % | 0 of 5 |
| **H**, top 50% | $+0.1$, $-1.3$, $-3.1$, $-3.0$, $-3.0$ % | 1 of 5 |
| **I**, top 1% | $+37.7$, $+6.3$, $+8.5$, $+33.0$, $+8.4$ % | **5 of 5**, four clear of 1.0 |

**The divergence is confined to the right tail**, which is the operating point of most
applications and the only depth where $\mathrm{LR}^{+}$ rises much above 1. There the
decontaminated score is ahead in every bin. Through the middle and the far end it trails by a
few per cent, significantly but slightly — at the median $\mathrm{LR}^{+}$ runs 1.19–1.39 for
both scores, so the largest deficit is $1.23 \to 1.19$, a loss of 0.04, against $1.95 \to 2.69$
at the top. Both deficits grow with GC, which locates whatever the decontaminated model
discards in GC-rich sequence, where the regional adjustment varies most. Panel G is held near
1.0 by arithmetic rather than by measurement; the section below G's cell is why.

Two footnotes on I. In the units an analyst reads, the matched rate makes its rows a rescaling
of the per-bin **precision** ratio: the decontaminated score's calls are correct **1.30 times**
as often as published's in the most AT-rich bin and **1.02 times** as often in the most
GC-rich. Those are *precision* ratios, smaller than the odds ratios I plots, and quoting one
for the other is the easiest error to make here. And I does not contradict C: auPRC integrates
over the whole recall axis while I probes only its top, and the two PR curves cross between the
1st and 50th percentile — which makes **auPRC the wrong summary for a constraint score**, the
top ~1% being the entire use case.

### The genome-wide budget, which is not one of the panels

At the same 10,051-call budget, published finds 5,485 positives and the decontaminated score
4,396 (precision $0.546 \to 0.437$). That cost is real and it is **not** evidence against the
correction, because the quantity it is measured in is the one the correction removes: this
truth set's positives are GC-rich (base rate 8.3% in the lowest GC bin, 63.9% in the highest)
and published Gnocchi is precisely the score that calls GC-rich sequence constrained. Its
pooled advantage is its GC bias working as an enhancer detector, so judging the correction by
this table is circular. Which is why every panel is computed **within** a GC bin.

**What Fig. 5F and this figure establish together:**

> Decontaminating removes the GC dependence of how often Gnocchi fires, and with it the GC
> dependence of how much it finds (A, B). Where the score is actually used — the top of the
> ranking — it raises what a call is worth (F, I). It leaves untouched the GC dependence of how
> much a hit is worth, which is signal-to-noise and a property of the data (C).
""")
...
# windows identical to Fig. 5E's apart from the enhancer-overlapping half.
...
# QUIET: it reprints the population preamble the cell above already showed, and its own
# numbers print in the caption cell at the end. D.quiet says why that is a context manager
# rather than a `verbose` flag.
with D.quiet():
...
# (that is D).
# QUIET for the same reason: the caption cell prints these per-bin rows once, with the
# Wilson intervals the panels actually draw.
with D.quiet():
...
code(r"""
# PANELS F AND I -- THE TOP-1% ROW -- one construction read twice: threshold_used per
# (bin, score) is what F draws, and the LR+ measured at those thresholds is what I draws. The
# calling rate is matched WITHIN each bin, so the threshold is a construction and not a number
# anyone applies -- NOT A and B's operating point, which is one fixed global threshold whose
# per-bin freedom IS the bias. See data.LAX_CALL_RATES and data._bin_thresholds.
#
# QUIET, and no per-bin print here: all three depths land in one table a few cells down,
# which is the only place the numbers should be read off.
with D.quiet():
...
with D.quiet():
    withinbin_50 = D.threshold_metrics(truth_set="lax", call_rate=CALL_RATE_MID,
                                       match_within_bin=True) if NEUTRAL_WINDOWS_BED else None
    gains_wb_50 = D.paired_deltas(truth_set="lax", call_rate=CALL_RATE_MID,
                                  n_bootstrap=500, seed=0, match_within_bin=True,
                                  metric="lr_pos") if NEUTRAL_WINDOWS_BED else None

    withinbin_99 = D.threshold_metrics(truth_set="lax", call_rate=CALL_RATE_HIGH,
                                       match_within_bin=True) if NEUTRAL_WINDOWS_BED else None
    gains_wb_99 = D.paired_deltas(truth_set="lax", call_rate=CALL_RATE_HIGH,
                                  n_bootstrap=500, seed=0, match_within_bin=True,
                                  metric="lr_pos") if NEUTRAL_WINDOWS_BED else None
""")
...
if curves_s8 is not None:
    # NINE PANELS, ONE QUESTION -- what debiasing does to DISCOVERY. Two rows of two slots,
    # a slot being either a stack or one full-height panel:
    #
    ...
    #
    # THE TWO STACKS ARE ROW-ALIGNED BY CALLING RATE -- D with G at 99%, E with H at 50%, F
    # with I at 1% -- so each ratio panel sits beside the threshold it was measured at, and
    # reading down either stack scans the score axis. Ordered left tail -> median -> right
    # tail, which is why the rate descends.
    #
    # C keeps its full height because auPRC/r spans only ~1.2-1.6 and a wide short panel would
    # flatten the decline that is the point of drawing it. Each ratio panel keeps its OWN y
    # range: at 99% the ratio is pinned near 1.0 by arithmetic, so one shared axis would draw
    # it flat.
    #
    # Letters moved on 2026-09-11; CLAUDE.md carries the map from the old ones, which captions
    # and git history predating that date still use.
    fig = plt.figure(figsize=(13.0, 11.5))
    # THE COLUMN GAP IS A SPACER COLUMN rather than a wspace, so its width is stated in
    # inches rather than inferred: 13.0 / (2 + 0.76) gives 4.71 in per panel and 3.58 in of
    # gap, which is what holds A/B's right-hand LR+ label, C's label, and the ratio stack's
    # group label. HEIGHT RATIOS favour row 2, which carries three-panel stacks against row
    # 1's two: 1 : 1.35 on 11.5 in gives each stacked panel ~2.2 in, and a ratio curve with
    # error bars is what suffers first when a row is short.
    gs = fig.add_gridspec(2, 3, width_ratios=[1.0, 0.76, 1.0],
                          height_ratios=[1.0, 1.35], wspace=0.0, hspace=0.22)
    gsAB = gs[0, 0].subgridspec(2, 1, hspace=0.34)
    axA = fig.add_subplot(gsAB[0, 0])
    axB = fig.add_subplot(gsAB[1, 0], sharex=axA)
    axC = fig.add_subplot(gs[0, 2])
    # hspace 0.42 on both stacks: every panel here carries a multi-line rotated y label, and
    # on a third-height row those collide with the panel above at 0.35.
    gsDEF = gs[1, 0].subgridspec(3, 1, hspace=0.42)
    axD = fig.add_subplot(gsDEF[0, 0])
    axE = fig.add_subplot(gsDEF[1, 0], sharex=axD)
    axF = fig.add_subplot(gsDEF[2, 0], sharex=axD)
    gsGHI = gs[1, 2].subgridspec(3, 1, hspace=0.42)
    axG = fig.add_subplot(gsGHI[0, 0])
    axH = fig.add_subplot(gsGHI[1, 0], sharex=axG)
    axI = fig.add_subplot(gsGHI[2, 0], sharex=axG)
    ...
    if tm_s8 is not None:
        ...
        # call is worth anything? -- which is why the two scores get two panels and not two
        # curves. Both share both y ranges, so the shapes compare by eye. No `deltas`: the
        # object is the relation between two curves for ONE score, which a paired
        # published-vs-decontaminated interval does not describe. Wilson bars.
    ...
    # D, E AND F ARE THREE QUANTILES OF ONE DISTRIBUTION, per GC bin and per score. If the
    # published threshold climbs with GC at all three, the bias is a shift of the WHOLE score
    # within a bin rather than a distortion of one tail -- which is what licenses reading G, H
    # and I as three views of one ranking. None of the three uses labels, and E doubles as a
    # calibration check, its threshold being each bin's median z.
    if withinbin_99 is not None:
        panels.panel_bin_thresholds(axD, withinbin_99, D.LAX_CALL_RATES[2],
                                    show_xlabel=False)
        axD.tick_params(labelbottom=False)
    if withinbin_50 is not None:
        panels.panel_bin_thresholds(axE, withinbin_50, D.LAX_CALL_RATES[1],
                                    show_xlabel=False)
        axE.tick_params(labelbottom=False)
    if withinbin_s8 is not None:
        panels.panel_bin_thresholds(axF, withinbin_s8, D.LAX_CALL_RATE)

    # C IS THE THRESHOLD-FREE VERDICT and the figure's hinge: I reports a gain at the top 1%,
    ...
    # hit an enhancer throughout. RATIOS, not levels: levels invite a cross-bin reading of each
    # score against itself, which is A and B's job, where these ask only whether
    # decontaminating helps IN a bin. Each `gains_*` must carry metric="lr_pos", whose `delta`
    # is the ODDS ratio -- a precision ratio is bounded by 1/precision and so not comparable
    # across bins. Recall is drawn beside none of them: with the rate matched within a bin
    # recall = lift x k at constant k, a constant rescaling, so it earns A and B instead, where
    # the global threshold lets k vary 45.7x.
    #
    # Magnitudes GROW down the column as the rate falls, because at 99% both scores' precision
    # is within about a percent of prevalence. Read G for sign, H and I for magnitude.
    if gains_wb_99 is not None:
        panels.panel_lr_ratio(axG, gains_wb_99, D.LAX_CALL_RATES[2], show_xlabel=False)
        axG.tick_params(labelbottom=False)
    if gains_wb_50 is not None:
        panels.panel_lr_ratio(axH, gains_wb_50, D.LAX_CALL_RATES[1], show_xlabel=False)
        axH.tick_params(labelbottom=False)
    if gains_wb_s8 is not None:
        panels.panel_lr_ratio(axI, gains_wb_s8, D.LAX_CALL_RATE)

    # ONE GROUP LABEL FOR THE RATIO STACK, carrying the direction of the ratio. Each panel's
    # own y label states only what varies between the three -- the calling rate -- because a
    # rotated label is as tall as its longest line is wide, and at three panels to a row there
    # is no room for both. The direction has to stay somewhere in the artwork, these panels
    # having no legend, so it is stated once for the stack it applies to.
    #
    # PLACED FROM THE AXES' OWN BBOXES rather than at a guessed figure coordinate, so it
    # tracks any later change to the gridspec. The 0.17 offset is measured: it clears the
    # panel letters by 45.8 px and sits 47 px clear of the left column's axes, inside the
    # spacer column.
    if gains_wb_s8 is not None:
        _stack = [ax.get_position() for ax in (axG, axH, axI)]
        fig.text(min(b.x0 for b in _stack) - 0.17,
                 0.5 * (max(b.y1 for b in _stack) + min(b.y0 for b in _stack)),
                 "decontaminated / published", rotation=90, va="center", ha="center",
                 fontsize=panels.AXIS_LABEL_FONTSIZE - 2)

    for ax, lab, x in ((axA, "A", -0.26), (axB, "B", -0.26), (axC, "C", -0.16),
                       # D-I SIT FURTHEST OUT, at -0.40, because their y labels are PINNED
                       # (panel_bin_thresholds says why) and a pinned label cannot be pushed
                       # aside by a letter. -0.40 leaves +20.7 px to the label and +13.3 px
                       # to the tick labels. The letters then overhang the figure's left edge
                       # by 3.5 px, which is harmless: resave_ai saves with
                       # bbox_inches="tight", so the box grows to include them.
                       (axD, "D", -0.40), (axE, "E", -0.40), (axF, "F", -0.40),
                       (axG, "G", -0.40), (axH, "H", -0.40), (axI, "I", -0.40)):
...
code(r"""
# THE THREE DEPTHS IN ONE TABLE, which is the figure's whole argument and is easier to read
# here than off six stacked panels. It is the ONLY place D-I's numbers are printed: the cells
# that computed them ran under D.quiet(), because five builders each reprinting the population
# preamble and its own per-bin dump is about 110 lines, nearly all identical.
#
# EVERY COLUMN COMES FROM THE `gains_wb_*` FRAMES ALONE. paired_deltas carries both scores'
# thresholds and both their LR+ levels beside the ratio, so the left half of this table is
# what D, E and F draw and the right half is what G, H and I draw, and neither can drift from
# the artwork. The `withinbin_*` frames exist only to be handed to the panels.
#
# EXPECT THE MAGNITUDES TO GROW DOWN THE TABLE as the calling rate falls: at 99% both scores'
# precision is within about a percent of prevalence, so a ratio there is pinned near 1.0 by
# arithmetic. Read that block for its SIGN and the other two for size.
_depths = [("99%", "D / G", gains_wb_99),
           ("50%", "E / H", gains_wb_50),
           ("1%", "F / I", gains_wb_s8)]
if all(g is not None for _, _, g in _depths):
    print("Supporting Fig. 8 D-I: one comparison at three depths of each GC bin's own score")
    print("distribution. Thresholds are what D, E and F draw; the LR+ ratio is what G, H and I")
    print("draw. * marks a 95% paired interval clear of zero.\n")
    for _rate, _panels, _g in _depths:
        print(f"  top {_rate}  (panels {_panels})")
        print(f"    {'GC bin':<15}{'thr pub':>9}{'thr dec':>9}"
              f"{'LR+ pub':>9}{'LR+ dec':>9}{'ratio':>9}{'95% CI':>18}")
        for e in _g.sort("mid").iter_rows(named=True):
            ci = f"[{100 * e['ci_lo']:+6.1f},{100 * e['ci_hi']:+6.1f}]"
            star = "*" if (e["ci_lo"] > 0 or e["ci_hi"] < 0) else " "
            print(f"    ({e['lo']:.2f}, {e['hi']:.2f}]{'':<3}"
                  f"{e['threshold_published']:>9.3f}{e['threshold_scored']:>9.3f}"
                  f"{e['lr_pos_published']:>9.3f}{e['lr_pos_scored']:>9.3f}"
                  f"{100 * e['delta']:>+8.1f}%{ci:>18} {star}")
        _n = sum(1 for e in _g.iter_rows(named=True) if e["delta"] > 0)
        print(f"    favours the decontaminated score in {_n}/{_g.height} bins\n")
    print("  A sign holding across all three depths would say the two scores differ the same\n"
          "  way everywhere on the ranking. A sign that FLIPS says their precision-recall\n"
          "  curves cross, and between which depths -- which is where panel C's threshold-free\n"
          "  wash is averaging a gain against a loss.")
""")
...
et al. tested GC content, BGS and gBGC; Chen et al.'s regional model is fit on thirteen
features across four length scales, most never checked on neutral windows. The patch is
applied per symptom and the list of symptoms is open-ended.
...
score's GC behaviour only; repeating Fig. 5E against BGS and gBGC is the obvious next check,
...
if curves_s8 is not None:
    ...
    # PANEL C's POOLED PAIR ONLY. Its per-bin auPRC/r is already on the page, one line per
    # score, from pr_curves' own print further up; repeating it here as five rows per score was
    ...
    for c in curves_s8.values():
        print(f"  {c['display']:<42} pooled auPRC/r = {c['all']['aupr_norm']:.3f}")
...
if tm_s8 is not None:
    # The ONLY print of A and B's rows -- the builder that made them ran under D.quiet().
    print(f"\npanels A and B. Published held at Gnocchi >= {D.GNOCCHI_THRESHOLD:g}, Chen et")
    print("al.'s own threshold; the decontaminated score takes the threshold calling the SAME")
    print("fraction of the whole population. Unbalanced. call_rate uses no labels at all, so")
    print("it is the most robust number here.")
